...
Event.observe(document, "keydown", function (event) {
  ...
  if (event.keyCode == 69 && ctrlPressed && altPressed) {
    if (isEnglishOn == "false") {
      ...
      changeKeyBoard("english");
    } else {
      isEnglishOn = "false";
      $("unijoy").checked = "checked";
...
var OldTitle = "";
var OldContent = "";
function insertEnglish() {
  insertAtCursor("[Bswjk]" + $F("english") + "[/Bswjk]");
}
function insertLink(linkurl) {
  if (linkurl == " " || linkurl == null) {
  } else {
    insertAtCursor("[wjsK=" + linkurl + "][/wjsK]");
  }
}
function chkForm() {
  if (!$("title").present()) {
    alert("Please give post title... ");
    $("title").focus();
    return false;
  }
  if (!$("content").present()) {
